...
from _util__soen import dend_load_rate_array
...
def run_soen_sim(obj, **kwargs):
    ...
    # dendrite
    if type(obj).__name__ == 'dendrite':
        
        obj.time_params = {'dt': dt, 'tf': tf, 'time_vec': time_vec, 't_tau_conversion': 1e-9/obj.jj_params['tau_0']}
        
        # make drive signals
        obj = construct_dendritic_drives(obj)
        
        # step through time
        obj = dendrite_time_stepper(obj)
    ...
    if type(obj).__name__ == 'neuron':
        ...
        recursive_dendrite_initialization_and_drive_construction(obj.dend__nr_ni,tau_vec) # go through all other dendrites in the neuron's arbor
        
        # load rate arrays to all dendrites
        recursive_rate_array_attachment(obj.dend__nr_ni)
        
        # initialize all input synapses
        recursive_synapse_initialization(obj.dend__nr_ni,tau_vec,t_tau_conversion)
        
        # initialize all output synapses
        output_synapse_initialization(obj,tau_vec,t_tau_conversion)
        
        # initialize transmitter
        transmitter_initialization(obj,t_tau_conversion)
        
        # step through time        
        print('\nrunning neuron time stepper for {:4.2e}ns ({:d} time steps) ...'.format(time_vec[-1],len(tau_vec)))
        obj = neuron_time_stepper(obj,tau_vec,d_tau)  
        
        # add output data to dendrites for plotting and diagnostics
        recursive_dendrite_data_attachment(obj.dend__nr_ni,obj)
    ...
    if type(obj).__name__ == 'network':
        ...
        for neuron_key in obj.neurons:
            ...
            # initialize dendrites (phi_r vector) and make drive signals
            recursive_dendrite_initialization_and_drive_construction(obj.neurons[neuron_key].dend__nr_ni,tau_vec) # go through all other dendrites in the neuron's arbor
...
def recursive_dendrite_initialization_and_drive_construction(dendrite_object,tau_vec):
        
    # print('dend_name = {}'.format(dendrite_object.name))
    # time.sleep(0.2)
    ...
    for dendrite_key in dendrite_object.dendritic_inputs:
        recursive_dendrite_initialization_and_drive_construction(dendrite_object.dendritic_inputs[dendrite_key],tau_vec) 
        ...
        if hasattr(dendrite_object.synaptic_inputs[synapse_key],'input_signal'):
            if hasattr(dendrite_object.synaptic_inputs[synapse_key].input_signal,'input_temporal_form'):
                if dendrite_object.synaptic_inputs[synapse_key].input_signal.input_temporal_form == 'constant_rate':
                    ...
                    isi = (1/rate) * 1e9 # 1e-9 to convert to ns
...
def output_synapse_updater(neuron_object,time_index,present_time):
    for synapse_key in neuron_object.synaptic_outputs:
        ...
        if len(_st_ind) > 0:
            ...
            _st_ind = int(_st_ind[-1])
            if neuron_object.synaptic_outputs[synapse_key].spike_times_converted[_st_ind] <= present_time and (present_time - neuron_object.synaptic_outputs[synapse_key].spike_times_converted[_st_ind]) < neuron_object.synaptic_outputs[synapse_key].spd_duration_converted: # the case that counts    
...
def neuron_time_stepper(neuron_object,tau_vec,d_tau):
    ...
    for ii in range(len(tau_vec)-1):
        ...
        # update all input synapses and dendrites
        recursive_dendrite_updater(neuron_object.dend__nr_ni,ii,tau_vec[ii+1],d_tau)
        
        # update all output synapses
        output_synapse_updater(neuron_object,ii,tau_vec[ii+1])
        
        # check if ni loop has increased above threshold
        if neuron_object.dend__nr_ni.s[ii+1] >= neuron_object.integrated_current_threshold:
            
            neuron_object.dend__nr_ni.threshold_flag = True
            neuron_object.spike_times.append(tau_vec[ii+1])
            neuron_object.spike_indices.append(ii+1)
            ...
            if neuron_object.source_type == 'qd' or neuron_object.source_type == 'ec':
                ...
                num_samples = neuron_object.num_photons_out_factor*len(neuron_object.synaptic_outputs)
                random_numbers = rng.random(size = num_samples)
                
                photon_delay_tau_vec = np.zeros([num_samples])
                for qq in range(num_samples):
                    photon_delay_tau_vec[qq] = neuron_object.time_params['tau_vec__electroluminescence'][ ( np.abs( neuron_object.electroluminescence_cumulative_vec[:] - random_numbers[qq] ) ).argmin() ]
                                       
                # assign photons to synapses
                for synapse_name in neuron_object.synaptic_outputs:
                    neuron_object.synaptic_outputs[synapse_name].photon_delay_times__temp = []
                    
                while len(photon_delay_tau_vec) > 0:
                    
                    for synapse_name in neuron_object.synaptic_outputs:
                        neuron_object.synaptic_outputs[synapse_name].photon_delay_times__temp.append(photon_delay_tau_vec[0])
                        photon_delay_tau_vec = np.delete(photon_delay_tau_vec, 0)
                       
                for synapse_name in neuron_object.synaptic_outputs:
                    _ind = ( np.abs( tau_vec[:] - ( tau_vec[ii+1] + np.min(neuron_object.synaptic_outputs[synapse_name].photon_delay_times__temp) ) ) ).argmin()
                    neuron_object.synaptic_outputs[synapse_name].spike_times_converted = np.append(neuron_object.synaptic_outputs[synapse_name].spike_times_converted,tau_vec[_ind])
                                            
            elif neuron_object.source_type == 'delay_delta':
                
                _ind = ( np.abs( tau_vec[:] - ( tau_vec[ii+1] + neuron_object.light_production_delay ) ) ).argmin()
    ...
    _t1 = time.time_ns()
    print('done running neuron time stepper. t_sim = {:7.5e}s\n'.format(1e-9*(_t1-_t0))) # {:7.5e}
...
def dendrite_time_stepper(dendrite_object):
    ...
    ib__vec = np.asarray(ib__list)
    _ind__ib = ( np.abs( ib__vec[:] - dendrite_object.bias_current ) ).argmin()
    phi_r__vec = np.asarray(phi_r__array[_ind__ib])    
    
    # convert to dimensionless time
    time_vec = dendrite_object.time_params['time_vec']
    tau_vec = time_vec*dendrite_object.time_params['t_tau_conversion']
    dendrite_object.time_params['tau_vec'] = tau_vec
    d_tau = dendrite_object.time_params['dt']*dendrite_object.time_params['t_tau_conversion']
    beta = dendrite_object.circuit_betas[-1]
    alpha = dendrite_object.alpha
    
    # calculate external flux drives
    phi_r_applied__vec = np.zeros([len(tau_vec)])
    for input_drive in dendrite_object.external_inputs:
        phi_r_applied__vec += dendrite_object.external_inputs[input_drive].drive_signal * dendrite_object.external_connection_strengths[input_drive]
        
    # initialize synapses
    for synapse_name in dendrite_object.synaptic_inputs:
        
        dendrite_object.synaptic_inputs[synapse_name]._phi_spd_memory = 0
        dendrite_object.synaptic_inputs[synapse_name]._st_ind_last = 0
        dendrite_object.synaptic_inputs[synapse_name].phi_spd = np.zeros([len(tau_vec)])
        
        dendrite_object.synaptic_inputs[synapse_name].spike_times_converted = np.asarray(dendrite_object.synaptic_inputs[synapse_name].input_signal.spike_times) * dendrite_object.time_params['t_tau_conversion']
        dendrite_object.synaptic_inputs[synapse_name].tau_rise_converted = dendrite_object.synaptic_inputs[synapse_name].tau_rise * dendrite_object.time_params['t_tau_conversion']
        dendrite_object.synaptic_inputs[synapse_name].tau_fall_converted = dendrite_object.synaptic_inputs[synapse_name].tau_fall * dendrite_object.time_params['t_tau_conversion']
        dendrite_object.synaptic_inputs[synapse_name].hotspot_duration_converted = dendrite_object.synaptic_inputs[synapse_name].hotspot_duration * dendrite_object.time_params['t_tau_conversion']
        dendrite_object.synaptic_inputs[synapse_name].spd_duration_converted = dendrite_object.synaptic_inputs[synapse_name].spd_duration * dendrite_object.time_params['t_tau_conversion']
        
    # step through time
    s = np.zeros([len(tau_vec)])
    phi_r = np.zeros([len(tau_vec)])
    print('\nrunning dendrite time stepper for {:4.2e}ns ({:d} time steps) ...'.format(time_vec[-1],len(tau_vec)))
    _t0 = time.time_ns()
    for ii in range(len(tau_vec)-1):
        ...
        # directly applied flux
        phi_r[ii+1] = phi_r_applied__vec[ii+1]
        # print('directly applied flux at this timestep = {}'.format(phi_r[ii+1]))
        
        # applied flux from synapses
        for synapse_name in dendrite_object.synaptic_inputs:
            
            # find most recent spike time for this synapse
            _st_ind = np.where( _pt > dendrite_object.synaptic_inputs[synapse_name].spike_times_converted[:] )[0]
            
            if len(_st_ind) > 0:
                
                _st_ind = int(_st_ind[-1])
                if dendrite_object.synaptic_inputs[synapse_name].spike_times_converted[_st_ind] <= _pt and (_pt - dendrite_object.synaptic_inputs[synapse_name].spike_times_converted[_st_ind]) < dendrite_object.synaptic_inputs[synapse_name].spd_duration_converted: # the case that counts    
                    _dt_spk = _pt - dendrite_object.synaptic_inputs[synapse_name].spike_times_converted[_st_ind]
                    _phi_spd = spd_response( dendrite_object.synaptic_inputs[synapse_name].phi_peak, 
                                            dendrite_object.synaptic_inputs[synapse_name].tau_rise_converted,
                                            dendrite_object.synaptic_inputs[synapse_name].tau_fall_converted,
                                            dendrite_object.synaptic_inputs[synapse_name].hotspot_duration_converted, _dt_spk)
                    
                    # to avoid going too low when a new spike comes in
                    if _st_ind - dendrite_object.synaptic_inputs[synapse_name]._st_ind_last == 1: 
                        _phi_spd = np.max( [ _phi_spd , dendrite_object.synaptic_inputs[synapse_name].phi_spd[ii] ])
                        dendrite_object.synaptic_inputs[synapse_name]._phi_spd_memory = _phi_spd
                    if _phi_spd < dendrite_object.synaptic_inputs[synapse_name]._phi_spd_memory:
                        dendrite_object.synaptic_inputs[synapse_name].phi_spd[ii+1] = dendrite_object.synaptic_inputs[synapse_name]._phi_spd_memory
                    else:
                        dendrite_object.synaptic_inputs[synapse_name].phi_spd[ii+1] = _phi_spd * dendrite_object.synaptic_connection_strengths[synapse_name]
                        dendrite_object.synaptic_inputs[synapse_name]._phi_spd_memory = 0
                    
                dendrite_object.synaptic_inputs[synapse_name]._st_ind_last = _st_ind
                        
            phi_r[ii+1] += dendrite_object.synaptic_inputs[synapse_name].phi_spd[ii+1]
        
        # find relevant entry in r_fq__array
        _ind__phi_r = ( np.abs( phi_r__vec[:] - phi_r[ii+1] ) ).argmin()
        i_di__vec = np.asarray(i_di__array[_ind__ib][_ind__phi_r])
        _ind__s = ( np.abs( i_di__vec[:] - s[ii] ) ).argmin()
        r_fq = r_fq__array[_ind__ib][_ind__phi_r][_ind__s]
        
        s[ii+1] = s[ii] * ( 1 - d_tau*alpha/beta) + (d_tau/beta) * r_fq
    
    _t1 = time.time_ns()
    print('done running dendrite time stepper. t_sim = {:7.5e}s\n'.format(1e-9*(_t1-_t0))) # {:7.5e}
    
    dendrite_object.output_data = {'s': s, 'tau_vec': tau_vec, 'phi_r': phi_r, 'time_to_solution': 1e-9*(_t1-_t0)}
    dendrite_object.s = s
    
    return dendrite_object

def synapse_time_stepper(synapse_object):
    
    time_vec = synapse_object.time_vec
    spike_times = synapse_object.input_signal.spike_times
    tau_rise = synapse_object.tau_rise
    tau_fall = synapse_object.tau_fall
    hotspot_duration = synapse_object.hotspot_duration
    spd_duration = synapse_object.spd_duration
    phi_peak = synapse_object.phi_peak
    
    _phi_spd_memory = 0
    _st_ind_last = 0
    phi_spd = np.zeros([len(time_vec)])
    print('\nrunning synapse time stepper for {:d} time steps ...'.format(len(time_vec)))
    _t0 = time.time_ns()
            ...
            if len(_st_ind) > 0:
                ...
                if spike_times[_st_ind] <= _pt and (_pt - spike_times[_st_ind]) < spd_duration: # the case that counts    
                    ...
                    if _st_ind - _st_ind_last == 1: 
                        _phi_spd = np.max( [ _phi_spd , phi_spd[ii] ])
                        _phi_spd_memory = _phi_spd
                    if _phi_spd < _phi_spd_memory:
                        phi_spd[ii+1] = _phi_spd_memory
                    else:
                        phi_spd[ii+1] = _phi_spd
                        _phi_spd_memory = 0
                    
                _st_ind_last = _st_ind
    
    _t1 = time.time_ns()
    print('done running synapse time stepper. t_sim = {:7.5e}s\n'.format(1e-9*(_t1-_t0))) # {:7.5e}
    
    synapse_object.output_data = {'phi_spd': phi_spd, 'time_to_solution': 1e-9*(_t1-_t0)}
