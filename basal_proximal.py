...
def gen_input(case='Predictive'):
    ...
    # basal input activity (one spike per 50ns)
...
def run_prox_basal_experiment(syn_struct,W,case,input_basal,input_proximal,input_empty):

    n1 = NeuralZoo(type='custom',synaptic_structure=syn_struct,weights=W,s_th=.5,tau_ni=500,tau_di=500,beta_ni=2*np.pi*1e3)
    n2 = NeuralZoo(type='custom',synaptic_structure=syn_struct,weights=W,s_th=.5,tau_ni=500,tau_di=500,beta_ni=2*np.pi*1e3)
...
def bas_prox_plot(net,neurons,input_basal,input_prox,title):
    ...
    for ii in range(2):
        ...
        print(spike_times)
        axs[ii].plot(spike_times,np.ones(len(spike_times))*neurons[ii].neuron.s_th,'xk', markersize=8, label=f'neuron fires')

        axs[ii].axhline(y = neurons[ii].neuron.s_th, color = 'purple', linestyle = ':',label='Firing Threshold')

        axs[ii].plot(input_prox.spike_arrays[1],np.zeros(len(input_prox.spike_arrays[1])),'xg', markersize=5, label='proximal input event')

    # axs[i].plot(net.t,phi_r,  label='phi_r (soma)')
    axs[0].plot(input_basal.spike_arrays[1],np.zeros(len(input_basal.spike_arrays[1])),'x',color='orange', markersize=5, label='basal input event')
